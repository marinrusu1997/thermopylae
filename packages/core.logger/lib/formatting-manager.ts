...
class FormattingManager {
	 ...
	 */
	public setCustomFormattingOrder(order: Formatter[]): void {
		...
		if (options.colorize) {
			...
			if (typeof options.colorize === 'object') {
				for (const label in options.colorize) {
					if (Object.hasOwn(options.colorize, label)) {
	...
	public formatterFor(module: string): Format {
		...
		for (const formatterName of this.formattingOrder) {
			const formatter = this.formatters.get(formatterName);
			if (!formatter) {
				throw createException(ErrorCodes.FORMATTER_NOT_REGISTERED, `Formatter '${formatterName}' from the order is not registered.`);
			}
	...
	private defineFormatters(): void {
		...
		this.formatters.set(DefaultFormatters.LABEL_FILTER, format((info) => (this.ignoredLabels.has(info['label'] as string) ? false : info))());
		this.formatters.set(
			DefaultFormatters.LABEL_LOG_LEVEL_FILTER,
			format((info) =>
				FormattingManager.LOG_LEVEL_TO_NUMBER[info.level as string] < FormattingManager.LOG_LEVEL_TO_NUMBER[this.levelForLabel[info['label'] as string]]
					? false
					: info
			)()
		);
