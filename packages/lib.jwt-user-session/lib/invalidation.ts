...
class InvalidationStrategy<Device extends DeviceBase, Location> {
	...
	public async generateRefreshToken(
		...
		(context as unknown as MutableSome<UserSessionMetaData<Device, Location>, 'expiresAt'>).expiresAt = UnixTimestampC(currentTimestamp + refreshTokenTtl);
	...
	public invalidateAccessToken(jwtAccessToken: IssuedJwtPayload): void {
		...
		this.options.invalidAccessTokensCache.upset(invalidationKey, null, SecondsC(invalidationTtl));
	}
	...
	public invalidateAccessTokensFromAllSessions(subject: Subject, jwtAccessTokenTtl: Seconds): void {
		...
		const invalidatedAt = chrono.unix();
