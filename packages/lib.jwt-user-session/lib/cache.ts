...
/** _In-memory_ cache where invalid access tokens are stored. */
interface InvalidAccessTokensCache {
	...
	/**
	 * Check for access token existence in the cache. <br/> If found, it means access token is no
	 * longer valid.
	 *
	 ...
	 * @param   accessToken Access token.
	 *
	 * @returns             Invalidation timestamp or _undefined_ if `accessToken` was not present
	 *   in the cache.
	 */
	get(accessToken: string): UnixTimestamp | null | undefined;
}

export type { InvalidAccessTokensCache };
