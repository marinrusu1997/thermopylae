			...
			if (e instanceof ValidationError) {
				res.status(HttpStatusCode.BadRequest).send({
					error: {
						...
						message: API_VALIDATOR.joinErrors(e.errors, 'object')
					}
				});
				return;
			}
...
apiRouter[API_SCHEMA.authenticate.method](
	API_SCHEMA.authenticate.path,
	userAgent,
	jsonBody,
	validateRequestBody,
	ipLocation,
	handler(async (req: Request<ObjMap, ResponseBody, RequestBody>, res: Response<ResponseBody>) => {
