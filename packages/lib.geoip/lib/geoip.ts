...
import type { IpLocation, IpLocationsRepository } from './repository/index.js';

/**
 * Geo IP locator which retrieves location of the IP address. <br/> Locator has a set of
 * repositories from where locations are fetched. Repositories are load balanced via **weighted
 * round robin algorithm**, and tried in order until some of them retrieves location.
 */
class GeoIpLocator {
	 ...
	 *
	 * > Notice that location might differ when invoking this function with the same `ip` multiple
	 * > times. <br/> This is caused by the way repositories are selected by load balancer in order to
	 * > retrieve location for that ip. <br/> If you need same location object at each invocation, you
	 * > can explicitly define `repo` from where to fetch location. Notice that location has
	 * > {@link IpLocation.REPOSITORY_ID} property which indicates from which repository it was
	 * > retrieved.
	 *
	 * @param   ip   Ip address.
