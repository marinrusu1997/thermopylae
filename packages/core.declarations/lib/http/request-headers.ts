...
export enum HttpRequestHeaderEnum {
	...
	A_IM = 'a-im',
	/**
	 * [Media type(s)](https://en.wikipedia.org/wiki/Media_type) that is/are acceptable for the response.
	 ...
	 * See [HTTP compression](https://en.wikipedia.org/wiki/HTTP_compression).
	 */
	 ...
	 * with [Origin](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields#origin-request-header).
	 */
	...
	/**
	 * Authentication credentials for [HTTP authentication](https://en.wikipedia.org/wiki/Basic_access_authentication).
	 */
	AUTHORIZATION = 'authorization',
	/**
	 * Used to specify directives that must be obeyed by all [caching mechanisms](https://en.wikipedia.org/wiki/Web_cache#Cache-control)
	...
	/**
	 * The length of the request body in [octets](https://en.wikipedia.org/wiki/Octet_(computing)) (8-bit bytes).
	 */
	CONTENT_LENGTH = 'content-length',
	/**
	 * A [Base64](https://en.wikipedia.org/wiki/Base64)-encoded binary [MD5](https://en.wikipedia.org/wiki/MD5)
	 * sum of the content of the request body.
	 */
	CONTENT_MD5 = 'content-md5',
	/**
	 * The [Media type](https://en.wikipedia.org/wiki/Media_type) of the body of the request
	 ...
	 * (in "HTTP-date" format as defined by [RFC 7231 Date/Time Formats](http://tools.ietf.org/html/rfc7231#section-7.1.1.1)).
	 */
	DATE = 'date',
	/**
	...
	FROM = 'from',
	/**
	 * The domain name of the server (for [virtual hosting](https://en.wikipedia.org/wiki/Virtual_hosting)),
	 * and the [TCP port](https://en.wikipedia.org/wiki/List_of_TCP_and_UDP_port_numbers) number
	 * on which the server is listening. <br/>
	 * The [port](https://en.wikipedia.org/wiki/Port_(computer_networking)) number may be omitted
	 * if the port is the standard port for the service requested. <br/>
	 ...
	 * The *HTTP2-Settings* header field is a connection-specific header field that includes parameters
	 * that govern the HTTP/2 connection, provided in anticipation of the server accepting the request to upgrade.
	 */
	...
	/**
	 * Allows a *304 Not Modified* to be returned if content is unchanged,
	 * see [HTTP ETag](https://en.wikipedia.org/wiki/HTTP_ETag_.
	 */
	...
	/**
	 * Initiates a request for [cross-origin resource sharing](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing)
	 * (asks server for [Access-Control-*](https://en.wikipedia.org/wiki/List_of_HTTP_header_fields#access-control-response-headers) response fields).
	 */
	ORIGIN = 'origin',
	/**
	 ...
	 * See [Referer](https://en.wikipedia.org/wiki/HTTP_referer).
	 */
	REFERER = 'referer',
	/**
	 * The transfer encodings the user agent is willing to accept: the same values as for the
	 * response header field *Transfer-Encoding* can be used, plus the "trailers" value
	 * (related to the "[chunked](https://en.wikipedia.org/wiki/Chunked_transfer_encoding)" transfer method)
	 * to notify the server it expects to receive additional fields in the trailer after the last, zero-sized, chunk. <br/>
	 * Only **trailers** is supported in HTTP/2.
	 */
	TE = 'te',
	/**
	 * The Trailer general field value indicates that the given set of header fields
	 * is present in the trailer of a message encoded with [chunked transfer coding](https://en.wikipedia.org/wiki/Chunked_transfer_encoding).
	 */
	TRAILER = 'trailer',
	/**
	 * The form of encoding used to safely transfer the entity to the user.
	 * [Currently defined methods](https://www.iana.org/assignments/http-parameters/http-parameters.xhtml)
	 * are: [chunked](https://en.wikipedia.org/wiki/Chunked_transfer_encoding), compress, deflate, gzip, identity. <br/>
