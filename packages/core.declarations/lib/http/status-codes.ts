// oxlint-disable no-bitwise, prefer-literal-enum-member
/**
...
enum HttpStatusCode {
	 ...
	 * inappropriate headers would be inefficient. To have a server check the request's headers, a
	 * client must send Expect: 100-continue as a header in its initial request and receive a 100
	 * Continue status code in response before sending the body. The response 417 Expectation Failed
	 ...
	 *
	 * @see {@link https://tools.ietf.org/html/rfc8297}
	 */
	EarlyHints = 103,

	/** All `1xx` status codes. */
	INFORMATIONAL_RESPONSES = Continue | SwitchingProtocols | Processing | EarlyHints,
	 ...
	 * agent-driven content negotiation). For example, this code could be used to present multiple
	 * video format options, to list files with different filename extensions, or to suggest
	...
	/**
	 * SINCE HTTP/1.1 The requested resource is available only through a proxy, the address for
	 * which is provided in the response. Many HTTP clients (such as Mozilla and Internet Explorer)
	 ...
	 * parallel the behaviors of 302 and 301, but do not allow the HTTP method to change. So, for
	 ...
	 * is not usually used. Google Developers API uses this status if a particular developer has
	 * exceeded the daily limit on requests.
	 */
	PaymentRequired = 402,

	/**
	 * The request was valid, but the server is refusing action. The user might not have the
	 ...
	 * use cases do not require clients and search engines to purge the resource, and a "404 Not
