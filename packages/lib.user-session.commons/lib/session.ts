...
interface UserSessionOperationContext<Device extends DeviceBase, Location> {
	...
	readonly ip: string;
	/**
	 * Device from where session was created/accessed. <br/> Can be _undefined_ or _null_ if device
	 * is unknown.
	 */
	readonly device?: Nullable<DeepReadonly<Device>>;
	/**
	 * Location from where session was created/accessed. <br/> Can be _undefined_ or _null_ if
	 * location is unknown.
	 */
...
export type { SessionId, Subject, DeviceBase, UserSessionOperationContext, UserSessionMetaData };
