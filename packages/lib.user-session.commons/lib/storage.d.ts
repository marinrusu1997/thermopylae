...
import type { DeviceBase, SessionId, Subject, UserSessionMetaData } from './session.js';
...
> {
	 ...
	 *
	 * > **IMPORTANT!** <br/> It's highly advisable to hash `sessionId` before storing it in the
	 * > database, especially if RDBMS is used.
	 *
	 ...
	 *
	 * @returns           User session meta data or _null_ / _undefined_ if not found.
	 */
	 ...
	 *
	 * @returns         Session id with the session metadata. <br/> When `subject` has no active
	 *   sessions, returns an empty map.
	 */
