...
import { AccountStatus, type AccountWithTotpSecret, type FailedAuthenticationModel } from '@thermopylae/lib.authentication';
...
describe(`${FailedAuthenticationsMysqlRepository.name} spec`, { timeout: 2_500 }, function suite() {
	...
	describe(`${FailedAuthenticationsMysqlRepository.prototype.readRange.name} spec`, () => {
		...
		it('returns authentications starting from unix timestamp', async () => {
			...
			/* READ THEM */
			expect(await failedAuthenticationRepository.readRange(firstAccount.id, now)).toStrictEqual([]);
		...
		it('returns authentications ending with unix timestamp', async () => {
			...
			/* READ THEM */
			expect(await failedAuthenticationRepository.readRange(firstAccount.id, undefined, now - 101)).toStrictEqual([]);
		...
		it('returns authentications starting from and ending to unix timestamps', async () => {
			...
			/* READ THEM */
			expect(await failedAuthenticationRepository.readRange(firstAccount.id, now - 102, now - 101)).toStrictEqual([]);
