...
import { ClientModule, DevModule, type ObjMap } from '@thermopylae/core.declarations';
...
beforeEach(async () => {
	const { mysql } = inject('connectionOptions' as never) as { mysql: MySqlConnectionDetails };
	...
	try {
		...
		await Promise.all(
			(tables as ObjMap[]).map(async (table) => {
