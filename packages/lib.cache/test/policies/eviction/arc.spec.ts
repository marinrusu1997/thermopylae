...
describe(`${colors.magenta(ArcEvictionPolicy.name)} spec`, () => {
	it('should work', () => {
		...
		policy.setDeleter((evictedEntry) => {
			...
			expect(evictableEntry[SEGMENT_TYPE_SYM]).toBeUndefined();
			expect(evictableEntry.value).toBeUndefined();
			expect(evictableEntry.key).toBeUndefined();

			EVICTED_KEYS.push(evictedKey);
		});

		function get(key: string): EvictableCacheEntry<string, string> {
		...
		policy.onHit(get('1-B2')); // T1(2) <-> T2(1)

		/* Available -> Full */
		policy.onSet(backend.set('2-B2', '2-B2')); // T1(1) <-> T2(1)
		policy.onHit(get('2-B2')); // T1(2) <-> T2(0)

		policy.onSet(backend.set('3-B2', '3-B2')); // T1(1) <-> T2(0)
		policy.onHit(get('3-B2')); // T1(2) <-> T2(0) with eviction
		...
		policy.onHit(get('6-B1')); // T1 reorder from [_, _, 6, 7] to [_, _, 7, 6]
		...
		policy.onHit(get('7')); // T1 reorder from [7, 6, 8, 9] -> [6, 8, 9, 7]
		...
		policy.onHit(get('7')); // T1 [_, 10] <-> T2 [7, _]
		policy.onHit(get('10-B2')); // T1 [_, _] <-> T2 [10, 7]
		...
		policy.onHit(get('7')); // T1 [] <-> T2 [7, 12, 11, 10]
		policy.onHit(get('11')); // T1 [] <-> T2 [11, 7, 12, 10]
		...
		policy.onHit(get('20-B2')); // T1 [_, 21] <-> T2 [20, _]
		policy.onHit(get('21')); // T1 [_, _] <-> T2 [21, 20]
		...
		policy.onHit(get('23')); // T1 [_, 24] <-> T2 [23, 21]
