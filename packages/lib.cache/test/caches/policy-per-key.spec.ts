...
describe(`${PolicyPerKeyCache.name.magenta} spec`, () => {
	describe(`${PolicyPerKeyCache.prototype.get.name.magenta} & ${PolicyPerKeyCache.prototype.set.name.magenta} spec`, () => {
		it("calls 'onHit' hook for policies specified for that particular entry and emits events", () => {
			...
			const policy1 = new PolicyMock<string, string, unknown>();
			const policy2 = new PolicyMock<string, string, unknown>();
			const policy3 = new PolicyMock<string, string, unknown>();
			...
			let insertEventKey: string | null = null;
			let insertEventValue: string | null = null;
			cache.on(CacheEvent.INSERT, (key, value) => {
				insertEventKey = key;
				insertEventValue = value;
			});
			let updateEventKey: string | null = null;
			let updateEventValue: string | null = null;
			...
			expect(updateEventKey).toBeNull();
			expect(updateEventValue).toBeNull();
			...
			(policy3.methodBehaviours.get('onHit') ?? PolicyMock.DEFAULT_METHOD_BEHAVIOUR).returnValue = EntryValidity.NOT_VALID;
			...
			(policy2.methodBehaviours.get('onHit') ?? PolicyMock.DEFAULT_METHOD_BEHAVIOUR).returnValue = EntryValidity.NOT_VALID;
			...
			(policy1.methodBehaviours.get('onHit') ?? PolicyMock.DEFAULT_METHOD_BEHAVIOUR).returnValue = EntryValidity.NOT_VALID;
