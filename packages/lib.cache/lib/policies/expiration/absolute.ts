...
> {
	...
	public onSet(entry: ExpirableCacheEntry<Key, Value>, options?: ArgumentsBundle): void {
		...
		const { expiresAfter, expiresFrom } = options;
		if (AbsoluteExpirationPolicy.isExpirable(expiresAfter)) {
			AbsoluteExpirationPolicy.setEntryExpiration(entry, expiresAfter, expiresFrom);
		}
	}
	...
	public onUpdate(entry: ExpirableCacheEntry<Key, Value>, options?: ArgumentsBundle): void {
		...
		if (options.expiresAfter === INFINITE_EXPIRATION) {
			entry[EXPIRES_AT_SYM] = types.SOFT_DELETE; // entry is no longer expirable, logical deletion
			return;
		}
	...
	protected evictIfExpired(entry: ExpirableCacheEntry<Key, Value>): EntryValidity {
		if (entry[EXPIRES_AT_SYM] && entry[EXPIRES_AT_SYM] <= chrono.unix()) {
	...
	protected static isExpirable(expiresAfter?: Seconds): expiresAfter is Seconds {
		return expiresAfter != null && expiresAfter !== INFINITE_EXPIRATION;
	}
}
