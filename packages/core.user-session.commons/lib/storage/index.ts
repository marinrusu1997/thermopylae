...
{
	private static readonly NO_ACTIVE_SESSIONS: ReadonlyMap<SessionId, types.Any> = new Map();
	...
	public constructor(options: UserSessionRedisStorageOptions<MetaData>) {
		...
		RedisClientInstance.on(ConnectionType.SUBSCRIBER, 'message', (channel, message) => {
			...
			RedisClientInstance.subscriber.unsubscribe(channel).catch((error) => {
			...
			RedisClientInstance.client.lrem(this.activeSessionsKey(subject), 1, sessionId).catch((error) => {
				logger.error(`Failed to remove '${sessionId}' from the list of active sessions of the subject '${subject}'.`, error);
			});
		});
	}
	...
	public async readAll(subject: Subject): Promise<ReadonlyMap<SessionId, MetaData>> {
		...
		for (let i = 0; i < activeSessions.length; i++) {
			...
			refreshTokenToActiveSessions.set(activeSessionIds[i], this.options.serializer.deserialize(activeSession));
		}
