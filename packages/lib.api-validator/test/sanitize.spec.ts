...
describe(`${ApiValidator.prototype.sanitize.name} spec`, () => {
	it('sanitizes string with xss in tag', () => {
		...
		const sanitizedString = apiValidator.sanitize(stringWithXss);
	...
	it('sanitizes json with xss', () => {
		...
		apiValidator.sanitize(jsonWithXss);
	...
	it('skips specified paths and does not sanitize them', () => {
		...
		apiValidator.sanitize(jsonWithXss, new Set<string>(['str1', 'obj.str1', 'obj.arr.[0]']));
