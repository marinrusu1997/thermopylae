...
class Collection<Document extends DocumentContract<Document>> implements Iterable<Document> {
	...
	public constructor(options?: Partial<CollectionOptions<Document>>) {
		...
		this.originality = options.documentsOriginality == null ? DocumentOriginality.ORIGINAL : options.documentsOriginality;
	...
	public get indexes(): IndexedKey<Document>[] {
	 ...
	 */
	public insert(documents: Document | readonly Document[]): void {
		if (!Array.isArray(documents)) {
			documents = [documents] as readonly Document[];
		}
		...
		if (this.originality === DocumentOriginality.CLONE) {
			documents = Processor.clone(documents as readonly Document[]);
		}

		this.storage.insert(documents as readonly Document[]);
	 ...
	 * 	</code></pre>
	 */
	public find(query?: Nullable<Query<Document>>, options?: Partial<FindOptions<Document>>): Document[] {
	 ...
	 * 	</code></pre>
	 */
	public replace(query: Query<Document>, replacement: Document, options?: Partial<ReplaceOptions<Document>>): Document[] {
		const matches = this.findAndDelete(query, options);

		if (matches.length > 0 || (matches.length === 0 && options && options.upsert)) {
	 ...
	 * 	</code></pre>
	 */
	public update(query: Query<Document>, update: ObjMap, options?: Partial<UpdateOptions<Document>>): Document[] {
		// oxlint-disable-next-line no-array-method-this-argument
		const matches = this.find(query, options);

		let original: Optional<Array<Document>> = undefined;
	 ...
	 */
	public delete(query: Query<Document>, options?: Partial<DeleteOptions<Document>>): Document[] {
	...
	public clear(): void {
		...
		this.notifier.sink.next({
			...
			// @ts-expect-error They will be assigned later (maybe, I don't know for sure) sure)
			documents: null
		});
	}
	 ...
	 */
	public map<MappedDocument>(mapper: Mapper<Document, MappedDocument>, options?: Partial<IndexOptions<Document>>): MappedDocument[] {
	 ...
	 */
	public createIndexes(...keys: IndexedKey<Document>[]): void {
	 ...
	 */
	public dropIndexes(...keys: IndexedKey<Document>[]): void {
		if (keys.length === 0) {
	...
	private findAndDelete(query: Query<Document>, options?: Partial<FindOptions<Document>>): Document[] {
		// oxlint-disable-next-line no-array-method-this-argument
		const matches = this.find(query, options);

		if (matches.length > 0) {
			for (const match of matches) {
				this.storage.remove(PK_INDEX_NAME, match[PK_INDEX_NAME]);
			}
	...
	private static validateDocuments<DocumentType>(validator: AjvType, documents: readonly DocumentType[]): void | never {
