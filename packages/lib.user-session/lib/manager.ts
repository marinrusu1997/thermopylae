import { type MutableSome, type Seconds, SecondsC, UnixTimestampC } from '@thermopylae/core.declarations';
import type { DeviceBase, ReadUserSessionHook, SessionId, Subject, UserSessionOperationContext } from '@thermopylae/lib.user-session.commons';
import { chrono } from '@thermopylae/lib.utils';
import { createHash } from 'node:crypto';
import safeUid from 'uid-safe';
import { ErrorCodes, createException } from './error.js';
import type { UserSessionMetaData } from './session.js';
import type { UserSessionsStorage } from './storage.js';

/** Hooks called when session is renewed. <br/> Mainly can be used for logging purposes. */
interface RenewSessionHooks {
	/**
	 * Hook called when making an attempt to renew user session, but it was renewed already from
	 * current NodeJs process.
	 *
	 ...
	 * after {@link UserSessionTimeouts.oldSessionAvailabilityAfterRenewal} seconds. <br/> In case
	 * the deletion of the old session will fail, this hook will be called with that error.
	 *
	 * @param sessionId Id of the session.
	 * @param error     Error that caused failure of the old session deletion.
	 */
	onOldSessionDeleteFailure(sessionId: string, error: Error): void;
}

interface UserSessionTimeouts {
	/**
	 * This timeout defines the amount of time in seconds a session will remain active in case there
	 * is no activity in the session, closing and invalidating the session upon the defined idle
	 * period since the last HTTP request received by the web application. <br/> If you do not need
	 * idle session feature, do not set this option. <br/> **Defaults** to _undefined_.
	 */
	readonly idle?: Seconds;
	/**
	 * This timeout defines the amount of time in seconds since session creation after which the
	 * session ID is automatically renewed. Renewal happens automatically when user session is read
	 * by manager. <br/> Renewal consists in deletion of the old session and creation of a new one.
	 * <br/> If you do not need renewal session feature, do not set this option. <br/> **Defaults**
	 * to _undefined_.
	 */
	readonly renewal?: Seconds;
	/**
	 * This timeout defines the amount of time the old session will still be available after it was
	 * renewed. <br/> This timeout starts counting from renew session operation, and on elapse will
	 * delete the old session. <br/> Usually you will want to keep this timeout as small as possible
	 * to give a chance to requests that were issued before renew operation to finish successfully,
	 * and then invalidate old session. If {@link UserSessionManagerOptions.timeouts.renewal} option
	 * is not set, this option is ignored. <br/> **Required** when
	 * {@link UserSessionManagerOptions.timeouts.renewal} option is set. **Recommended** value is
	 * _5_.
	 */
...
interface UserSessionManagerOptions<Device extends DeviceBase, Location> {
	/**
	 * Length of the generated session id. <br/> Because base64 encoding is used underneath, this is
	 * not the string length. For example, to create a token of length 24, you want a byte length of
	 * 18. <br/> Value of this option should not be lower than 15. <br/>
	 *
	...
	/**
	 * Read user session hook. <br/> Defaults to hook which ensures that in case device is present
	 * in both context and session metadata, their _name_ and _type_ needs to be equal.
	 */
	readonly readUserSessionHook?: ReadUserSessionHook<Device, Location, UserSessionMetaData<Device, Location>>;
	/** Hooks called on session renew. <br/> Defaults to noop hooks. */
...
/**
 * Stateful implementation of the user sessions. <br/> Session data is stored in external storage
 * and client receives only it's id. <br/> Sessions are implemented in a such way, so that they can
 * be used in cluster or single-node infrastructures.
 *
...
class UserSessionManager<Device extends DeviceBase = DeviceBase, Location = string> {
	...
	/**
	 * Sessions that were renewed and are about to expire very soon. <br/> This acts as _atomic
	 * flag_ at the NodeJS process level, to mark the session as being renewed and prevent further
	 * renews on it. <br/>
	 */
	private readonly renewedSessions: Map<SessionId, NodeJS.Timeout | null>;

	/**
	 * @param options Options object. <br/> It should not be modified after, as it will be used
	...
	public async create(subject: Subject, context: UserSessionOperationContext<Device, Location>, sessionTtl?: Seconds): Promise<SessionId> {
		...
		(context as unknown as MutableSome<UserSessionMetaData<Device, Location>, 'expiresAt'>).expiresAt = UnixTimestampC(currentTimestamp + sessionTtl);
	...
	/**
	 * Read user session from external storage. <br/> When idle functionality is activated, this
	 * method might delete user session and throw an error to notify about this, if it was idle for
	 * more than {@link UserSessionManagerOptions.timeouts.idle} seconds. <br/> When renew
	 * functionality is activated, this method might create a new user session, and return it's id
	 * as the second part of the tuple. In case renewed session id is returned, it needs to be sent
	 * to application clients via 'Set-Cookie' header to replace the old session cookie. The old
	 * session will still be available for
	 * {@link UserSessionManagerOptions.timeouts.oldSessionAvailabilityAfterRenewal} seconds, so that
	 * older requests might complete successfully and client has time to refresh session id on it's
	 * side.
	 *
	 ...
	 * @param   context             Operation context.
	 *
	 * @returns           A tuple with the following parts: - session metadata - renewed session id
	 *   (if renewal took place)
	 *
	 ...
	 *   {@link ErrorCodes.USER_SESSION_EXPIRED} - session is accessed from a device which differs
	 *   from the one it was created - {@link ErrorCodes.USER_SESSION_EXPIRED} - session was expired
	 *   because of the idle timeout.
	 */
	public async read(
		subject: Subject,
		sessionId: SessionId,
	...
	/**
	 * Renew user session. <br/> Renewing consist from the following actions: 1. scheduling deletion
	 * of the old session in a very short amount of time 2. creating a new user session.
	 *
	 * @param   subject         Subject.
	 ...
	 * @param   context         Operation context.
	 *
	 * @returns                 The new user session id. <br/> When renew can't be performed, a log
	 *   message is printed and _null_ is returned.
	 */
	public async renew(
		subject: Subject,
		sessionId: SessionId,
		...
		try {
			...
			sessionMetaData.accessedAt = UnixTimestampC(RENEWED_SESSION_FLAG);
			await this.options.storage.updateAccessedAt(subject, sessionId, sessionMetaData);
		} catch (error) {
		...
		this.renewedSessions.set(
			...
			setTimeout(
				async () => {
					try {
						await this.delete(subject, sessionId);
					} catch (error) {
						this.options.renewSessionHooks.onOldSessionDeleteFailure(sessionId, error);
					}
				},
				(this.options.timeouts.oldSessionAvailabilityAfterRenewal ?? 1) * 1000
			)
		);

		// create new session
		return this.create(subject, context, SecondsC(sessionMetaData.expiresAt - sessionMetaData.createdAt));
	}
	...
	/**
	 * Hashes session id. <br/> Useful for logging purposes.
	 *
	 * @param   sessionId Session id.
	 *
	...
	private static fillWithDefaults<Dev extends DeviceBase, Loc>(options: UserSessionManagerOptions<Dev, Loc>): Required<UserSessionManagerOptions<Dev, Loc>> {
		...
		if (options.timeouts == null) {
			...
			if (options.timeouts.idle != null && (options.timeouts.idle <= 0 || options.timeouts.idle >= options.sessionTtl)) {
				throw createException(ErrorCodes.INVALID_IDLE_TIMEOUT, `Idle timeout needs to be >0 && <${options.sessionTtl}. Given ${options.timeouts.idle}`);
			}
	...
	private static readUserSessionHook<Dev extends DeviceBase, Loc>(
		...
		if (
			context.device &&
			sessionMetaData.device &&
			(context.device.type !== sessionMetaData.device.type || context.device.name !== sessionMetaData.device.name)
		) {
			throw createException(
...
export { UserSessionManager, type UserSessionManagerOptions, type UserSessionTimeouts, type RenewSessionHooks, RENEWED_SESSION_FLAG };
