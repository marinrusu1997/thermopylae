...
function generate(generationType = TokenGenerationType.CRYPTOGRAPHIC, length = UUID_DEFAULT_LENGTH): string {
	let alg: typeof v1 | typeof v4 | null = null;
	...
	for (let i = 0; i < iterations; i++) {
		alg(undefined, buffer as unknown as undefined, i * UUID_BUFFER_OFFSET);
	}
...
function fastUnSecureHash(obj: string | ObjMap): number {
	...
	for (let codePoint: number | undefined = 0, i = 0, len = obj.length; i < len; i++) {
		codePoint = obj.codePointAt(i);
		if (codePoint == null) {
			continue;
		}
		// oxlint-disable-next-line no-bitwise
		hashValue = (hashValue << 5) - hashValue + codePoint;
