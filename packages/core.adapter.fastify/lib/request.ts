...
import type { RawRequestDefaultExpression, RawServerBase, RawServerDefault } from 'fastify/types/utils.js';
...
> extends FastifyRequest<RouteGeneric, RawServer, RawRequest> {
	/**
	 * Property under which _FastifyDeviceDetector_ stores detected device. <br/> Notice that if you
	 * pass a fastify request object to {@link FastifyRequestAdapter} which already has this property
	 * with according device, _FastifyDeviceDetector_ won't compute this property again.
	 */
	[DEVICE_SYM]?: DeviceDetectorResult | null;
	/**
	 * Property under which location of the request is stored. <br/> _Important!_ Clients of the
	 * {@link FastifyRequestAdapter} have to set this property accordingly on fastify request object
	 * before passing it to adapter, as he won't compute it.
	 */
	[LOCATION_SYM]?: HTTPRequestLocation | null;
}

/**
 * Device detector which extracts device from _User-Agent_ header by using
 * [device-detector-js](https://www.npmjs.com/package/device-detector-js) dependency. <br/> Device
 * is stored in the Fastify request object under {@link DEVICE_SYM} symbol.
 */
...
class FastifyRequestAdapter<Body = ObjMap> implements HttpRequest<Body> {
	/**
	 * Device detector. <br/> You can assign you own **HttpDeviceDetector** implementation. <br/>
	 * Defaults to {@link FastifyDeviceDetector} which detects device based on _User-Agent_ header.
	 */
	...
	public cookie(name: string): string | undefined {
		// @ts-expect-error We embed cookie in the request cookies objectobject
...
export { FastifyRequestAdapter, type AdaptedFastifyRequest, FastifyDeviceDetector, LOCATION_SYM, DEVICE_SYM };
