...
import type { ClientContext, Result } from 'handy-redis/dist/generated/interface.js';
...
> {
	 ...
	 * @param   path Path in the JSON object. <br/> **Defaults** to root if not provided.
	 *
	 ...
	 * ## _**Syntax**_<pre>
	 *
	 * JSON.GET <key> [INDENT indentation-string] [NEWLINE line-break-string] [SPACE space-string]
	 * [NOESCAPE] [path ...] </pre>
	 *
	 * ## _**Description**_
	 *
	 * Return the value at _path_ in JSON serialized form.
	 *
	 * This command accepts multiple _path_ s, and defaults to the value's root when none are given.
	 *
	 * The following subcommands change the reply's format and are all set to the empty string by
	 * default: - _INDENT_ sets the indentation string for nested levels.
	 *
	 * - _NEWLINE_ sets the string that's printed at the end of each line.
	 * - _SPACE_ sets the string that's put between a key and a value.
	 *
	 * The _NOESCAPE_ option will disable the sending of \uXXXX escapes for non-ascii characters.
	 * This option should be used for efficiency if you deal mainly with such text. The escaping of
	 * JSON strings will be deprecated in the future and this option will become the implicit
	 * default.
	 *
	 * @param   key  Key under which JSON value is stored.
	 * @param   args Command arguments.
	 *
	 * @returns      The reply's structure depends on the number of paths. <br/> A single path
	 *   results in the value itself being returned, whereas multiple paths are returned as a JSON
	 *   object in which each path is a key.
	 */
	 ...
	 * ## _**Syntax**_<pre>
	 *
	 * JSON.SET <key> <path> <json> [NX | XX] </pre>
	 *
	 * ## _**Description**_
	 *
	 ...
	 * @param   args Command arguments.
	 *
	 * @returns      'OK' if executed correctly, or _null_ if the specified _NX_ or _XX_ conditions
	 *   were not met.
	 */
