 ...
 * > be initialized.
 */
type ObjectInitializer<Value> = (resource: Value, values: unknown[]) => void;
...
class ArrayObjectPool<T extends ObjMap = ObjMap> {
	...
	private readonly resources: ObjectResource<T>[];
	 ...
	 */
	public acquire(...args: unknown[]): Readonly<ObjectResource<T>> {
