...
import { type Node as DLLObjectPoolHandle, Yallist } from 'yallist';
import { ErrorCodes, createException } from '../error.js';

/**
 * Function which constructs the object when given a set of arguments. <br/> After object has been
 * created, it needs to be returned back.
 */
type ObjectConstructor<Value> = (...args: types.Any[]) => Value;

/**
 * Function which initializes object with new values (i.e. overwrites the object). <br/> Function
 * can do anything he want with object, such as removing properties, adding new ones, overwrite with
 * new values etc. <br/> After object has been initialized, it needs to be returned back.
 */
type ObjectInitializer<Value> = (resource: Undefinable<Value>, ...args: types.Any[]) => Value;

/**
 * Function called to destruct object. <br/> Usually such a function will nullify object values,
 * allowing GC to collect them. It might also free external hold resources, such as file
 * descriptors, tcp connections etc. <br/> After object has been destructed, it needs to be returned
 * back.
 */
...
interface DLLObjectPoolOptions<Value> {
	/**
	 * Capacity of the poll which represents the number of available objects. <br/> When number of
	 * used objects goes beyond _capacity_, and exception will be thrown, preventing acquiring new
	 * resources.
	 */
...
/**
 * Class which manages a pool of object resources. <br/> Internal implementation is based on 2
 * Doubly Linked Lists, one for free resources, and another for used ones.
 */
class DLLObjectPool<Value = ObjMap> {
	private readonly config: DLLObjectPoolOptions<Value>;

	private readonly free: Yallist<Undefinable<Value>>;

	private readonly used: Yallist<Undefinable<Value>>;

	public constructor(config: DLLObjectPoolOptions<Value>) {
		this.config = DLLObjectPool.assertConfig(config);
		this.free = Yallist.create(config.initialFreeShapes || []);
		this.used = Yallist.create();
	 ...
	 * <br/> After preemption, object will be put into used resources list. <br/> **This operation
	 * has O(1) complexity.**
	 *
	 ...
	 *
	 * @param   args           Arguments forwarded to object: - constructor (when resource is
	 *   acquired for the first time) - initializer (when resource is reused)
	 *
	 * @returns      Handle to object resource.
	 *
	 * @throws  {Exception}      When number of used resources goes beyond
	 *   {@link DLLObjectPoolOptions.capacity}.
	 */
	public acquire(...args: unknown[]): Readonly<DLLObjectPoolHandle<Value>> {
	...
	/**
	 * Release handle to object resource. <br/> **This operation has O(1) complexity.**
	 *
	...
	/**
	 * Release object resource <br/> **This method has O(n) complexity**, because we need to find
	 * according handle for that object resource before performing release.
	 *
	 * @param object Object resource.
	 */
	...
	/**
	 * Release all object resources. <br/> **This method has O(n) complexity**, because it needs to
	 * iterate over all used resources to free them (i.e. call their destructors).
	 */
...
export { DLLObjectPool };
export type { DLLObjectPoolOptions, DLLObjectPoolStats, ObjectConstructor, ObjectDestructor, ObjectInitializer, DLLObjectPoolHandle };
