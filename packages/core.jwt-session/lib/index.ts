...
import { InvalidAccessTokensMemCache } from './storages/access-tokens';
import { RefreshTokensRedisStorage } from './storages/refresh-tokens';
...
/**
 * Options for the session manager. <br/>
 *
 * {@link JwtSessionManagerOptions.invalidationOptions.invalidAccessTokensCache} is an optional property,
 * and defaults to {@link InvalidAccessTokensMemCache} implementation. <br/>
 * {@link JwtSessionManagerOptions.invalidationOptions.refreshTokensStorage} is an optional property,
 * and defaults to {@link RefreshTokensRedisStorage} implementation.
 */
type SessionManagerOptions = JwtSessionManagerOptions<JwtSessionDevice, HTTPRequestLocation> & {
	readonly invalidationOptions: PartialSome<
		InvalidationStrategyOptions<JwtSessionDevice, HTTPRequestLocation>,
		'invalidAccessTokensCache' | 'refreshTokensStorage'
	>;
};
...
interface JwtUserSessionMiddlewareOptions {
	...
	session: {
		...
		headers: {
			 ...
			 */
			access: HttpResponseHeader | string;
			/**
			 * Lowercase!!! // @fixme test it
			 */
			refresh: HttpResponseHeader | string;
		};
		persistent: boolean;
		/**
...
class JwtUserSessionMiddleware {
	...
	private readonly options: DeepRequired<DeepReadonly<JwtUserSessionMiddlewareOptions>>;
	...
	private static fillWithDefaults(options: JwtUserSessionMiddlewareOptions): DeepRequired<DeepReadonly<JwtUserSessionMiddlewareOptions>> {
		...
		if (options.jwt.invalidationOptions.invalidAccessTokensCache == null) {
			(options.jwt.invalidationOptions as MutableSome<
				InvalidationStrategyOptions<JwtSessionDevice, HTTPRequestLocation>,
				'invalidAccessTokensCache'
			>).invalidAccessTokensCache = new InvalidAccessTokensMemCache();
		}

		if (options.jwt.invalidationOptions.refreshTokensStorage == null) {
			(options.jwt.invalidationOptions as MutableSome<
				InvalidationStrategyOptions<JwtSessionDevice, HTTPRequestLocation>,
		...
		if (options.accessTokenExtractor == null) {
			options.accessTokenExtractor = JwtUserSessionMiddleware.extractAccessToken;
		}

		return options as DeepRequired<DeepReadonly<JwtUserSessionMiddlewareOptions>>;
	}
}
...
export { JwtUserSessionMiddleware };
