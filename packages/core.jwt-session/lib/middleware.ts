...
import {
	...
	type JwtPayload,
	type JwtSignOptions,
	ErrorCodes as JwtUserSessionErrorCodes,
	JwtUserSessionManager,
	type JwtUserSessionManagerOptions,
	type JwtVerifyOptions,
	TokenExpiredError
} from '@thermopylae/lib.jwt-user-session';
...
class JwtUserSessionMiddleware {
	...
	public async verify(req: HttpRequest, res: HttpResponse, verifyOptions?: JwtVerifyOptions, unsetSessionCookies = true): Promise<IssuedJwtPayload> {
		...
		let payloadCookie: string | undefined | null = null;
		...
		try {
			...
			if (
				...
				(error instanceof TokenExpiredError ||
					...
					(error instanceof Exception && error.code === JwtUserSessionErrorCodes.ACCESS_TOKEN_WAS_FORCIBLY_INVALIDATED))
			) {
				if (signatureCookie != null) {
