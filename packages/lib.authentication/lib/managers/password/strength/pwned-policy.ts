...
/**
 * Validator which ensures that password wasn't breached before. <br/> In order to determine this,
 * it makes an API call to [Have I Been Pwned](https://haveibeenpwned.com/Passwords).
 */
class PwnedPasswordValidator<Account extends AccountModel> implements PasswordStrengthPolicyValidator<Account> {
	...
	/**
	 * @param breachThreshold Password breach threshold. When this threshold is reached or exceeded,
	 *   password is considered unsecure. <br/> Usually you will want to set this as low as
	 *   possible, e.g. 1 or 2.
	 */
	...
	public async validate(password: string): Promise<void | never> {
		...
		const breachedPasswords = responseBody.split('\n');

		for (const breachedPassword of breachedPasswords) {
			const [entry, howOften] = breachedPassword.split(':');
