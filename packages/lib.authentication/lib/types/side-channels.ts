import type { AuthenticationContext, ChangePasswordContext } from './contexts.js';
import type { AccountModel } from './models.js';

/**
 * Sender used in order to notify client via email about different events happened in the
 * authentication process and account management.
 */
interface EmailSender<Account extends AccountModel> {
	/**
	 * Notify user about failure of the multi factor authentication. <br/> In case attacker guesses
	 * password, it will fail at mfa. User will be notified about this, and in case it wasn't him
	 * that failed mfa, he needs to be suggested to change password.
	 *
	...
	/**
	 * Notify user that his account has been disabled. <br/> Usually account will be disabled
	 * because of too many failed authentication attempts or explicitly by the admin.
	 *
	...
	/**
	 * Notify admin that user account has been disabled. <br/> Usually account will be disabled
	 * because of too many failed authentication attempts or explicitly by the admin.
	 *
	...
	/**
	 * Notify user that password of his account has been changed. <br/> In case attacker guesses his
	 * password or steals his session, user will receive this notification. <br/> User needs to be
	 * suggested to change password/recover account/enable mfa in case he doesn't recognize this
...
/**
 * Sender used in order to notify client via sms about different events happened in the
 * authentication process and account management.
 */
...
export type { EmailSender, SmsSender };
