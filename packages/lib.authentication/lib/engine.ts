import type { ObjMap, RequireSome, Seconds, Threshold, UnixTimestamp } from '@thermopylae/core.declarations';
import { compareTwoStrings } from 'string-similarity';
import uidSafe from 'uid-safe';
import type { TwoFactorAuthStrategy } from './authentication/2fa/interface.js';
import { AuthenticationOrchestrator } from './authentication/orchestrator.js';
import type { AuthenticationStatus } from './authentication/step.js';
import { TwoFactorAuthStep } from './authentication/steps/2fa-step.js';
import { AuthenticatedStep } from './authentication/steps/authenticated-step.js';
import { ChallengeResponseStep, type ChallengeResponseValidator } from './authentication/steps/challenge-response-step.js';
import { DispatchStep } from './authentication/steps/dispatch-step.js';
import { ErrorStep } from './authentication/steps/error-step.js';
import { GenerateTwoFactorAuthTokenStep } from './authentication/steps/generate-2fa-token-step.js';
import { GenerateChallengeStep } from './authentication/steps/generate-challenge-step.js';
import { PasswordStep } from './authentication/steps/password-step.js';
import { RecaptchaStep, type RecaptchaValidator } from './authentication/steps/recaptcha-step.js';
import { ErrorCodes, createException } from './error.js';
import { AuthenticationSessionRepositoryHolder } from './helpers/authentication-session-repository-holder.js';
import { type SecretEncryptionOptions, SecretEncryptor } from './helpers/secret-encryptor.js';
import { AccountManager } from './managers/account.js';
import { FailedAuthenticationsManager } from './managers/failed-authentications.js';
import { type PasswordHashingOptions, PasswordsManager } from './managers/password/index.js';
import type { PasswordStrengthPolicyValidator } from './managers/password/strength/policy.js';
import { TokenManager } from './managers/token.js';
import type { AuthenticationContext, BaseContext, ChangePasswordContext, SetTwoFactorAuthenticationContext } from './types/contexts.js';
import { AccountStatus, AuthenticationStepName } from './types/enums.js';
import type { OnAccountDisabledHook, OnAuthenticationFromDifferentContextHook, OnForgottenPasswordChangedHook, OnPasswordChangedHook } from './types/hooks.js';
import type { AccountModel, FailedAuthenticationModel, SuccessfulAuthenticationModel } from './types/models.js';
import type {
	AccountRepository,
...
import type { EmailSender, SmsSender } from './types/side-channels.js';
...
interface AuthenticationEngineThresholdsOptions {
	/**
	 * Number of allowed failed authentication attempts before account will be locked for
	 * {@link AuthenticationEngineTtlOptions.accountDisableTimeout} seconds.
	 */
...
interface AuthenticationEngineOptions<Account extends AccountModel> {
	...
	readonly hooks: {
		/**
		 * Hook called when authentication from different context (i.e. different device, location
		 * etc.) has been detected.
		 */
		readonly onAuthenticationFromDifferentContext: OnAuthenticationFromDifferentContextHook<Account>;
		/**
		 * Hook called when account has been disabled due to authentication error or explicitly by
		 * admin.
		 */
	...
	readonly validators: {
		...
		/**
		 * Challenge response validator. <br/> It is optional in case you won't use password-less
		 * authentication. <br/> If you provide it, challenge response authentication will be
		 * enabled.
		 */
	...
	readonly password: {
		...
		/**
		 * Password encryption options. <br/> Depending of this option value, following behaviours
		 * will occur: <br/>
		 *
		 * - _false_ - password hash will be stored in plaintext in the
		 *   {@link AccountModel.passwordHash} <br/>
		 * - _{@link SecretEncryptionOptions}_ - password hash will be encrypted before being stored in
		 *   the {@link AccountModel.passwordHash}
		 */
		...
		/**
		 * Password similarity threshold used when password is changed. When old and new password
		 * have a similarity equal or greater with this one, error will be thrown and password
		 * change process will be aborted. <br/> Ranges between [0, 1], **0** being completely
		 * different and **1** being completely similar.
		 */
...
class AuthenticationEngine<Account extends AccountModel> {
	...
	/**
	 * Authenticates user. <br/> <br/> Authentication process consists from multiple
	 * {@link AuthenticationStepName} and is performed in a state machine fashion. Authentication
	 * process state is persisted by {@link AuthenticationSession} and
	 * {@link FailedAuthenticationAttemptSession}. <br/> <br/> Transition to next state/step is
	 * announced to client via {@link AuthenticationStatus.nextStep}. Depending on this, he needs to
	 * provide additional data. <br/> When an error is encountered, it will be set into
	 * {@link AuthenticationStatus.error}. In case it is a {@link AuthenticationStatusError.soft}
	 * error caused by invalid credentials, user can continue authentication from
	 * {@link AuthenticationStatus.nextStep}. In case it is a {@link AuthenticationStatusError.hard}
	 * error, user needs to abort authentication. <br/> When password-less authentication by
	 * challenge-response mechanism is used, generated challenge will be sent by
	 * {@link AuthenticationStatus.token} property. <br/> When authentication completes successfully,
	 * only {@link AuthenticationStatus.authenticated} property will be present in the authentication
	 * status. <br/> The following {@link AuthenticationStatus} property combinations might be
	 * returned: <br/>
	 *
	 * - {@link AuthenticationStatus.nextStep} - previous authentication step was performed
	 *   successfully and client needs to move to next step <br/>
	 * - {@link AuthenticationStatus.nextStep} and {@link AuthenticationStatus.token} - _nonce_ was
	 *   generated successfully by {@link AuthenticationStepName.GENERATE_CHALLENGE} step and stored
	 *   in this property, client needs to move to next step
	 *   {@link AuthenticationStepName.CHALLENGE_RESPONSE} <br/>
	 * - {@link AuthenticationStatus.nextStep} and {@link AuthenticationStatusError.soft} - previous
	 *   authentication step failed and client needs to move to next step (might be the same as
	 *   previous one or differ) <br/>
	 * - {@link AuthenticationStatusError.hard} - authentication failed with hard error, account was
	 *   disabled and client needs to abort authentication. <br/>
	 * - {@link AuthenticationStatus.authenticated} - authentication completed successfully <br/><br/>
	 *
	 * Depending on the authentication next step, client needs to provide the following data: <br/>
	 *
	 * - **NONE** - authentication needs to be started by client by either providing
	 *   {@link AuthenticationContextInterface.password} or
	 *   {@link AuthenticationContextInterface.generateChallenge} <br/>
	 * - {@link AuthenticationStepName.CHALLENGE_RESPONSE} - client needs to provide encrypted nonce in
	 *   the {@link AuthenticationContextInterface.responseForChallenge} <br/>
	 * - {@link AuthenticationStepName.PASSWORD} - password was invalid and client needs to provide
	 *   another {@link AuthenticationContextInterface.password} <br/>
	 * - {@link AuthenticationStepName.RECAPTCHA} - invalid password/nonce was provided too many times
	 *   and client needs to provide another
	 *   {@link AuthenticationContextInterface.password}/{@link AuthenticationContextInterface.responseForChallenge}
	 *   and also {@link AuthenticationContextInterface.recaptcha} <br/>
	 * - {@link AuthenticationStepName.TWO_FACTOR_AUTH_CHECK} - client needs to provide 2fa token sent
	 *   to him via side channel in the
	 *   {@link AuthenticationContextInterface.twoFactorAuthenticationToken} <br/><br/>
	 *
	 * On successful authentication, {@link AuthenticationStatus.authenticated} property will contain
	 * user account.
	 *
	...
	/**
	 * Register user account. <br/> When account is registered, multi factor authentication is
	 * disabled. <br/> Depending on the {@link AccountModel.disabledUntil} value: <br/>
	 *
	 * - {@link AccountStatus.ENABLED} - account is enabled and stored by {@link AccountRepository}
	 * - {@link AccountStatus.DISABLED_UNTIL_ACTIVATION} - account is disabled until activation and
	 *   stored by {@link ActivateAccountSessionRepository}; token is generated and sent by
	 *   {@link EmailSender} <br/> In case any exception is thrown, registration fails and needs to
	 *   be repeated.
	 *
	 * @param  account           Account to be registered. <br/> Plaintext password needs to be
	 *   stored in the {@link AccountModel.passwordHash} property.
	 *
	 * @throws {Exception}         When: <br/> - provided {@link AccountModel.passwordHash} is weak.
	 *   <br/> - account contains duplicate fields. <br/>
	 */
	 ...
	 *   This scenario might occur because unactivated accounts are stored in a temporary storage
	 *   {@link ActivateAccountSessionRepository} and while user tries to activate his account,
	 *   another user was able to register an account with same fields and activate it before (very
	 *   unlikely, but still might happen). In case this exception is thrown, user account
	 *   registration is considered failed and user to register another account.
	 */
	...
	/**
	 * Enable/Disable two factor authentication on the user's account. <br/> When
	 * {@link SetTwoFactorAuthenticationContext.password} is given, it will be verified against the
	 * account one. If it is not valid, Exception with {@link ErrorCodes.INCORRECT_PASSWORD} will be
	 * thrown. If invalid password was provided too many times, Exception with
	 * {@link ErrorCodes.ACCOUNT_DISABLED} will be thrown. <br/>
	 *
	 * > **Important!** This method has authorization implications. It needs to be called only by
	 * > authenticated users for their account.
	 *
	 * @param   accountId           Account id.
	 * @param   enabled             Whether it's enabled or not.
	 * @param   context             Operation context.
	 *
	 * @returns           Result of the {@link TwoFactorAuthStrategy.onTwoFactorAuthEnabled} method.
	 *   <br/> Please check the result type of the
	 *   {@link AuthenticationEngineOptions.twoFactorAuthStrategy} that you've chosen.
	 *
	 * @throws  {Exception}           With code:<br/> - {@link ErrorCodes.ACCOUNT_NOT_FOUND} when
	 *   account id is not valid <br/> - {@link ErrorCodes.ACCOUNT_DISABLED} when account is
	 *   disabled. <br/> - {@link ErrorCodes.INCORRECT_PASSWORD} when provided password is not
	 *   valid.
	 */
	public async setTwoFactorAuthEnabled(accountId: string, enabled: boolean, context?: SetTwoFactorAuthenticationContext): Promise<ObjMap | null> {
		const account = await this.accountManager.readById(accountId);

		if (context != null && !(await this.verifyPassword(context.password, account, context))) {
			throw createException(
		...
		let hookResult: ObjMap | null = null;
	 ...
	 *
	 * > **Important!** This method has authorization implications. It needs to be called only by
	 * > authenticated users for their account or by admin.
	 *
	 * @param accountId    Account id.
	 * @param startingFrom Starting timestamp.
	 * @param endingTo     Ending timestamp.
	 */
	public getFailedAuthentications(accountId: string, startingFrom?: UnixTimestamp, endingTo?: UnixTimestamp): Promise<FailedAuthenticationModel[]> {
	 ...
	 * @param endingTo     Ending timestamp.
	 */
	public getSuccessfulAuthentications(accountId: string, startingFrom?: UnixTimestamp, endingTo?: UnixTimestamp): Promise<SuccessfulAuthenticationModel[]> {
	 ...
	 *
	 * @throws  {Exception}          With code:<br/> - {@link ErrorCodes.ACCOUNT_DISABLED} when
	 *   invalid password was provided too many times.
	 */
	...
	/**
	 * Changes user password. <br/> After successful changing, {@link OnPasswordChangedHook} will be
	 * called and then email notification will be sent to user by {@link EmailSender}. <br/> In case
	 * any exception is thrown, operation is considered failed.
	 *
	 ...
	 *   {@link ErrorCodes.SIMILAR_PASSWORDS} when {@link ChangePasswordContext.oldPassword} is too
	 *   similar with {@link ChangePasswordContext.newPassword} <br/> -
	 *   {@link ErrorCodes.WEAK_PASSWORD} when {@link ChangePasswordContext.newPassword} is too weak.
	 *
	 * > **Important!** This method has authorization implications. It needs to be called only by
	 *   > authenticated users and they can change only their password.
	 */
	...
	/**
	 * Creates forgot password session and starts account recovery procedure. <br/> Creates forgot
	 * password token and sends it via specified `sideChannel`. <br/> If account has
	 * {@link AccountModel.pubKey}, token will be encrypted before being sent to user. In this case
	 * user needs to decrypt token with his Private Key and send plaintext token back to server.
	 * <br/> In case any exception is thrown, operation is considered failed.
	 *
	 * > **Important!** When account not found err is thrown, REST API should not respond with error,
	 * > in order to prevent user enumeration, i.e it should respond with ok (something like token was
	 * > sent to specified email, even if account doesn't exist).
	 *
	 * @param   accountUniqueFieldName            Name of the account field, value of which user
	 *   remembers.
	 ...
	 *   {@link ErrorCodes.UNKNOWN_CREATE_FORGOT_PASSWORD_SESSION_SIDE_CHANNEL} when invalid
	 *   `sideChannel` was provided.
	 */
	...
	/**
	 * Change forgotten password. <br/> On successful password change,
	 * {@link OnForgottenPasswordChangedHook} will be called. <br/> In case any exception is thrown,
	 * operation is considered failed, and user needs to start again account recovery (i.e. obtain a
	 * new forgot password token). This restriction is imposed in order to prevent replay attacks.
	 * The only exception from this rule is when {@link ErrorCodes.SESSION_NOT_FOUND} error code is
	 * thrown, as in this case forgot password token wasn't invalidated.
	 *
	 * @param  token                 Forgot password token issued by
	 *   {@link AuthenticationEngine.createForgotPasswordSession} procedure. <br/> Token needs to be
	 *   in plaintext.
	 * @param  newPassword           New account password.
	 *
	 * @throws {Exception}             With code: <br/> - {@link ErrorCodes.SESSION_NOT_FOUND} when
	 *   token is not valid <br/> - {@link ErrorCodes.ACCOUNT_NOT_FOUND} when account not found
	 *   <br/>
	 *
	 *   - {@link ErrorCodes.ACCOUNT_DISABLED} when account is disabled <br/> -
	 ...
	 *
	 * > **Important!** This operation should be done only by admin, or by application's business
	 * > logic.
	 *
	 * @param accountId Account id.
	 * @param until     Unix timestamp until account will be disabled. <br/> Use
	 *   {@link UnixTimestamp} or {@link AccountStatus.DISABLED_UNTIL_ACTIVATION} to disable account
...
export { AuthenticationEngine };
export type { AuthenticationEngineOptions, AuthenticationEngineThresholdsOptions, AuthenticationEngineTtlOptions, AccountToBeRegistered };
