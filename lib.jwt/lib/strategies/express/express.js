...
function getDefaultFailureHandler(logger) {
	return async function onFailureHandler(err, status, res) {
		if (err && status === null) {
			// soft error
			tryToLogError(err, 'Error while authenticating (non jwt related)', logger);
			res.status(500).send(`Server Error. Authentication failed. ${err.message}`);
		} else if (err && status === false) {
			// token validation error
			tryToLogError(err, 'Error while authenticating (jwt related)', logger);
			res.status(401).send(`Unathorized: ${err.message}`);
		} else {
			throw new Error('Invalid err & status prametters combination');
		}
	};
}
...
function JwtAuthMiddleware(auth, opts) {
	...
	let middleware = async (req, res, next) => {
		...
		if (!token) {
			await onFailure(new Error('Jwt not present in request'), null, res);
			return;
		}
		try {
