...
describe('Account registration spec', () => {
	...
	it('registers a new account using explicit registration options', async () => {
		await AuthServiceInstance.register(defaultRegistrationInfo, { isActivated: true, useMultiFactorAuth: true }); // do not trigger deletion timer
	...
	it('registers a new account with default registration options when they are not explicitly provided', async () => {
		await AuthServiceInstance.register(defaultRegistrationInfo); // activate default is false, it will trigger deletion timer
		const account = await basicAuthServiceConfig.entities.account.read('username');
		...
		await AuthServiceInstance.activateAccount(activationToken.token); // this will cancel delete account timer
	});
	...
	it('fails to register new account if it is registered already', async () => {
		...
		await AuthServiceInstance.register(defaultRegistrationInfo, { isActivated: true }); // do not place delete timer
		try {
			await AuthServiceInstance.register(defaultRegistrationInfo, { isActivated: false });
	...
	it('fails to register new account if provided password is weak (owasp validator)', () => {
		return AuthServiceInstance.register({ ...defaultRegistrationInfo, password: 'weak-password' })
	...
	it('activates account using token sent via email at the registration', async () => {
		await AuthServiceInstance.register(defaultRegistrationInfo, { isActivated: false });
		const activationToken = JSON.parse(basicAuthServiceConfig['side-channels'].email.outboxFor(defaultRegistrationInfo.email)[0].html as string);
		await AuthServiceInstance.activateAccount(activationToken.token);
	...
	it('activate account will not fail if delete activate account session deleteion will fail', async () => {
	...
	it("doesn't activate account if provided token is not valid", async () => {
		await AuthServiceInstance.register(defaultRegistrationInfo, { isActivated: false });
		try {
			await AuthServiceInstance.activateAccount('invalid-token');
